...
"""What about amximum video lenght to post on different platfoms and how to handle exceeding limit? Upload first (max lenht ) 15 sec of a video and repost first phtots"""
...
BOT_NAME="pleasure_teasers_bot"

# Configuration settings
BLOCK_MESSAGES_WITHOUT_TEXT = False  # Set to True to block messages without text
ANALYZE_IMAGES = False  # Set to True to analyze images when text is empty
BYPASS_TWEET_GENERATION_EMPTY = True  
...
CHANNELS_TO_MONITOR = [
    '@OnlyTop01','@hellaway','@myerotic',-1002747842341,
]
...
async def generate_and_post_tweet(text, media_paths, dir_name):
    ...
    # Check if text is empty and handle according to configuration
    text_is_empty = not text or len(text.strip()) == 0
    

    logger.info(f"Beginning to extract content to aggregated file")
    # 1. First extract content to aggregated file
    # Use analyze_images setting based on whether text is empty
    analyze_images = ANALYZE_IMAGES
    aggregated_content = await extract_content_to_aggregated_file(
        text, media_paths, str(dir_path), 
        analyze_urls=False, 
        analyze_images=analyze_images
    )
    ...
    filter_prompt = (
        "Is the following content PURELY promotional without valuable information? Content should be classified as promotional if it contains:\n"
        "1. Pure advertising with no valuable content (e.g., 'Want more than just photos? Click here for OnlyStars')\n"
        "2. Spam messages with fake interactions (e.g., 'Nika sent you a message: I'm ready to show you everything')\n"
        "3. Gambling/casino promotions (e.g., 'Streamer bet 40₽ and won 44,400₽ on GETX')\n"
        "4. Fake AI trading bot promotions (e.g., 'AI trading bot makes $20-30k monthly')\n"
        "5. Pure affiliate/referral link spam with no content\n"
        "Content should NOT be classified as promotional if it contains:\n"
        "1. Specific model information (name, age, height, eye color, nationality, etc.)\n"
        "2. Descriptive content about models or their content\n"
        "3. Relevant information about the adult entertainment industry\n"
        "4. Any valuable descriptive content, even with promotional elements\n"
        "5. OnlyFans links (indicates model material is available)\n"
        "6. Mentions of downloading content (indicates valuable media is available)\n"
        "7. Content that includes model details AND promotional links\n"
        "8. Adult content descriptions or categories\n\n"
        "IMPORTANT: Content with OnlyFans links or download mentions likely contains valuable model material and should be allowed, even if promotional.\n\n"
    ...
    if BYPASS_TWEET_GENERATION_EMPTY and text_is_empty:
        logger.info("Bypassing tweet generation for empty messages")
        tweet_text = ""
    else:
        ...
        prompt_text = (
            "Rewrite the following content as an engaging, sensual Twitter post for an adult entertainment channel. "
            ...
            "2. PRESERVE all essential information about models (age, height, eye color, nationality, etc.)\n"
            "3. REMOVE dashboard links but KEEP the insight from the data\n"
            "4. If there's no text information about models, add sensual, flirty comments assuming attractive people in the images\n"
            "5. Make the tone erotic and seductive but tasteful\n"
            "6. Use sensual emojis and language that's appropriate for adult content\n"
            "7. Focus on the beauty and appeal of the models/subjects\n"
            "8. Keep it engaging and mysterious to encourage interaction\n\n"
            "Content: {content}"
        )
        ...
        tweet_text = await tweet_generation(
            ...
            model="gpt-4o-2024-11-20",
            system_content="You are a sensual adult content blogger creating engaging, erotic tweets. Be seductive, flirty, and mysterious while maintaining tasteful boundaries. Focus on the beauty and appeal of models/subjects. When no text is provided, create content based on media analysis or general sensual themes.",
            temperature=0.8,
            top_p=1.0,
            max_tokens=900,
            presence_penalty=0.0,
    ...
    if filter_result == "no":
        ...
        telegram_channel = "@PleasureTease" # e.g., "@mychannel"
        if telegram_channel:
            try:
